...
export default async function InquiryPage({ searchParams }: InquiryPageProps) {
  ...
  if (!from || !to || !date) {
    return (
      <div className="container mx-auto text-center mt-10">
        <h1 className="text-2xl font-bold text-white">Lütfen bir arama yapın.</h1>
        <Link href="/" className="text-rose-500 hover:text-rose-400 mt-4 inline-block">
          Ana Sayfaya Dön
        </Link>
      </div>
    );
  ...
  // API'den veriyi doğru fonksiyon adıyla ve doğru parametrelerle çekiyoruz.
    ...
    <div className="container mx-auto py-8">
      <div className="mb-6 px-4 md:px-0">
        ...
        <p className="text-white/70">
          <span className="font-semibold">{from}</span> → <span className="font-semibold">{to}</span> için{" "}
      ...
      {trips.length > 0 ? (
        ...
        <div className="text-center py-10 px-6 rounded-lg border border-white/10 bg-gray-900/80 backdrop-blur-sm">
          <h2 className="text-xl font-semibold text-white">Uygun Sefer Bulunamadı</h2>
          <p className="text-white/60 mt-2">
            Aradığınız kriterlere uygun bir sefer bulunamadı. Lütfen farklı bir tarih veya güzergah deneyin.
          </p>
          <Link href="/" className="mt-6 inline-block bg-rose-600 text-white font-bold py-2 px-4 rounded-md hover:bg-rose-500 transition-colors">
            Yeni Arama Yap
          </Link>
        </div>
      )}
